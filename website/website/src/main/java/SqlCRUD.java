...
public class SqlCRUD {
    ...
    public SqlCRUD(String connectionString, String user, String password) throws SQLException {
        companyCrud = new CompanyCRUDImp(connectionString, user, password);
        productCrud = new ProductCRUDImp(connectionString, user, password);
        createTables(connectionString, user, password);
    }

    public SqlCRUD(String user, String password) throws SQLException {
        this("jdbc:mysql://localhost:3306/AdminDB", user, password);
    }
    ...
    private static JsonObject enrich(JsonObject source, String key, String value) {
    ...
    private void createTables(String connectionString, String user, String password) throws SQLException {
    ...
    public JsonObject registerCompanyCRUD(JsonObject data) throws SQLException {
        return companyCrud.createCRUD(data);
    }

    public JsonObject registerProductCRUD(JsonObject data) throws SQLException {
    ...
    private static class CompanyCRUDImp implements CRUD {
        ...
        public CompanyCRUDImp(String connectionString, String user, String password) {
        ...
        public JsonObject createCRUD(JsonObject data) {
            ...
            try (Connection con = DriverManager.getConnection(jdbcUrl, user, password)) {
                try (PreparedStatement stmt = con.prepareStatement(CREATE_COMPANY_CMD)) {
                    ...
                    stmt.setString(6, data.getString(Keys.SERVICE_FEE.key));
                ...
                response = enrich(data, "status", data.getString(Keys.COMPANY_NAME.key) + " CompanyRegistered");
            } catch (SQLException e) {
            	response = enrich(data, "status", e.getMessage());
            }
            return response;
        }
    ...
    private static class ProductCRUDImp implements CRUD {
        ...
        public ProductCRUDImp(String connectionString, String user, String password) {
            this.jdbcUrl = connectionString;
            this.user = user;
            this.password = password;
        }
        ...
        public JsonObject createCRUD(JsonObject data) {
            ...
            try (Connection con = DriverManager.getConnection(jdbcUrl, user, password)) {
                try (PreparedStatement stmt = con.prepareStatement(CREATE_PRODUCT_CMD)) {
                    stmt.setString(1, data.getString(Keys.PRODUCT_ID.key));
                    stmt.setString(2, data.getString(Keys.COMPANY_NAME.key));
                ...
                response = enrich(data, "status", data.getString(Keys.PRODUCT_ID.key) + " ProductRegistered");
            } catch (SQLException e) {
            	response = enrich(data, "status", e.getMessage());
            }
            return response;
